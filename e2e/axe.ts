...
// suite is relied on to hold: the landing page prints its words over a
// photograph (src/app/[locale]/(public)/page.tsx, #26), and they are legible
// only because they sit on an opaque card stacked above it. Make that card
// translucent, drop a word straight onto the image, or put text over a
// gradient anywhere else, and every text node there moves from violations to
// incomplete. Reading only violations would then go green BECAUSE the check
// stopped working.
