...
- **The bucket needs a CORS rule for uploads.** The browser PUTs straight to a
  ...
  differ. Production (#24) should narrow it to its own domain. Since #101 the rule
  also allows `GET` with a `Range` header and exposes `Content-Range`,
  `Accept-Ranges`, `Content-Length` and `ETag`: the R360 reader (A13) fetches
  single frames out of an archive already in the bucket through a presigned GET, and
  learns the archive's size from `Content-Range`. A bucket created before 09.09.2026
  needs the rule re-applied — re-run the CORS step of `scripts/bootstrap-dev.sh` or
