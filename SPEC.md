...
- **G2** File name = content hash; served with `max-age=31536000, immutable` (two
  exceptions, both immutable all the same and recorded in §9: the R360 archive keyed by
  its ETag, and its frames keyed by set and ordinal).
...
- **G4** User files never pass through the application server (upload via presigned URL;
  ...
  never reads the archive and the one core must not decode 120 frames; decision of
...
- `works` (#72): `id`, `user_id` (cascade — a work is profile content), `name`,
  ...
  width, start frame, ring flattening; cue points later, #107), Zod and `CHECK` pinned to
  each other as in #39.
...
- `files`: `id`, `user_id`, `sha256`, `size_bytes`, `kind`, `parent_file_id`, `ext`,
  ...
  narrows it to the largest kept representation of every asset and leaves the R360 archive
  out, below).
  ...
  Its frames (A13, #68, decision of 09.09.2026) are the second exception to the hash-named
  key: kinds `r360-1600|r360-800`, `parent_file_id` = the archive's row, keys
  ...
  `(user_id, parent_file_id, kind)` excludes the `r360-*` kinds. They are uploaded to
  `staging/` under the reservation of #30 like any upload and copied under the set's keys
  on save, so a walked-away set is swept by the same walk and the same lifecycle rule. A
  set is never rewritten — a new archive is a new set — so G2's immutable caching holds,
  and a work's set is deleted by prefix. The A9 limit and the #69 meter are one number: it
  counts the 1600 set and leaves the archive out — an archive is free storage of any size,
  ten works per profile (A12; the risk in §10).
...
| Staged uploads are accounted only while their reservation window is open                          | The A9 quota counts bytes reserved at presign (#30). Past the window a walked-away upload stops counting and is swept on that user's next presign; the bucket's `staging/` lifecycle rule is the outer bound, and S3 expiration cannot go below one day. Decision of 05.09.2026. R360 frame sets stage the same way (#102).                                                                                                                                                                                                                                                                                                                                    |
| 100 MB/page-view is an estimate, not a measurement                                                | Verify with telemetry after launch.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| R360 frames are encoded by the owner's browser, not by sharp (G4's exception, A13)                | Encoder output differs between browsers, and the server cannot re-derive a frame without reading the archive. Accepted 09.09.2026: the one core must not decode 120 frames per work, and the archive stays as uploaded, so a server-side pass remains possible later. The server verifies count, size caps and a sample of WebP headers on save. The archive itself is outside the A9 limit (A12 "any size"): free storage bounded only by ten works per profile — to revisit with the paid-model conversation.                                                                                                                                                |
| R360 frame uploads are presigned without a length (#102)                                          | A frame's size is unknowable when the set's 2N URLs are minted, so its PUT is the one upload whose Content-Length the signature does not pin. The reservation counts the set at its ceilings (1 MiB and 384 KiB per frame), the save refuses any frame past its ceiling and discards the whole staged set at once, and the `staging/` lifecycle rule bounds what a client that never saves can leave. Residual, accepted 09.09.2026: staged bytes bounded by the uploader's bandwidth for up to a day. The fix on the table is a presigned POST policy with `content-length-range`, a new dependency (§7 "ask first") to verify against the real bucket first. |
