...
| A4  | Profile: display name (1–80 characters) and photo (JPEG/PNG/WebP ≤ 10 MB), from which WebP variants at 512 px and 128 px are produced, at quality 95 — the profile's own header shows the 512 one, sharp on a dense screen (#65, wanted by Dawid on 13.09.2026).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
- `files`: `id`, `user_id`, `sha256`, `size_bytes`, `kind`, `parent_file_id`, `ext`,
  ...
  **An object has one owner** (decision of 08.09.2026): keys written since #72 are
  ...
  work — or an account (#34) — is deleted by prefix. A variant's key is its original's with
  the extension replaced by its size and encoding — `-512q95s.webp` is 512 px at quality 95
  with sharp-YUV colour — because an immutable name must never receive new bytes: encoding a
  picture differently gives it a new name (#65). Variants written before 13.09.2026 are named
  by the size alone (`-512.webp`), and their rows carry those keys (#49).
  A bucket per user was ruled out: OVHcloud allows 100 per project (1,000 on request), names
  unique across OVHcloud.
...
| A change of e-mail tells a signed-in account whether another address is registered                | `changeEmail` answers a taken address with the same 200 and no mail, but mails the approval for a free one to the requester's own inbox (#10), and only then voids their earlier approval link. It takes a verified account; three requests an hour per IP, not per account. Sign-up then sign-in, or sign-up's own timing, tells the same without an account, but registers the free address and mails it. Accepted by Dawid on 13.09.2026: a probe only confirms an account here, and a studio's address is usually public on its own site (profiles here show none). Closing it needs a hook before the library's early return. Named in the review of #197; dates from #10.                                                                                    |
