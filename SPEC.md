...
- `files`: `id`, `user_id`, `sha256`, `size_bytes`, `kind`, `parent_file_id`, `ext`,
  ...
  that encode to identical bytes would collide there (#120). They are uploaded to
  ...
  set is never rewritten — a new zip is a new set — so G2's immutable caching holds,
